...
def add_contact_tk(contacts, name, phone, email, frame_contact_list):
    ...
    ttk.Label(frame_contact_list, text=contact["phone"]).grid(column=2, row=row)
    ttk.Label(frame_contact_list, text=contact["email"]).grid(column=3, row=row)
...
def main():
    ...
    frame_contact_list = ttk.Frame(root, padding="3 12 3 12")
    ...
    ttk.Entry(frame_add_contact, width=10, textvariable=email).grid(column=2, row=2, sticky=N)
    ttk.Entry(frame_add_contact, width=10, textvariable=phone).grid(column=3, row=2, sticky=E)
    ...
    ttk.Button(frame_add_contact,
               text="Añadir",
               command=lambda: add_contact_tk(contacts, name.get(), email.get(), phone.get(), frame_contact_list)
